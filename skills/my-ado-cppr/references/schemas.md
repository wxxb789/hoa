# Schemas — probe, plan, apply result

Machine-checkable JSON shapes consumed and emitted by `lib/run.py`. The SKILL.md
keeps the decision rules; this file is the full reference.
...
- `approve: true` — **ADO only**. Self-approve via `az repos pr set-vote`. Often fails on policy-protected branches; see [ado.md](ado.md).
